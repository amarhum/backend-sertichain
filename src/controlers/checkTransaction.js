...
const modelCheckTransaction = require("../models/checkTransaction");
...
if (!nomorSertifikat) {
  nomorSertifikat =
    ...
    // Pengecekan hash di blockchain
    const hashStartTime = performance.now();
    const isValid = await modelCheckTransaction.checkTransaction(data);
    const hashEndTime = performance.now();
    console.log(`lama waktu pengecekan hash di blockchain: ${(hashEndTime - hashStartTime).toFixed(2)} ms`);
    
    const totalEndTime = performance.now();
    console.log(`Total waktu proses: ${(totalEndTime - totalStartTime).toFixed(2)} ms`);
    
    return res.status(200).json({
      message: isValid.exists
      ? "Sertifikat ASLI: Hash ditemukan di blockchain."
      : "Sertifikat PALSU: Hash tidak ditemukan di blockchain.",
      isValid,
      data,
    });
  } catch (error) {
